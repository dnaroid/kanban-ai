...
export function TaskDrawerDetails({task, onUpdate, columnName}: TaskDrawerDetailsProps) {
  ...
  const handleImproveDescription = async () => {
    // Open an OpenCode session to improve the task
    // We'll use the task tool to launch a general agent
    try {
      await window.api.opencode.startSession({
        initialPrompt: `Please improve the following user story/task description. Make it follow best practices (Gherkin syntax if applicable, clear acceptance criteria). Context: This is a task in a kanban board for a software project.\n\nTask Title: ${task.title}\n\nCurrent Description:\n${editedDescription}`,
      })
    } catch (error) {
    ...
    <div className="flex flex-col h-full bg-[#0B0E14] animate-in fade-in duration-300">
      <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-8">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <button
                ...
                className="p-1.5 text-purple-400 hover:text-white hover:bg-purple-500/10 rounded transition-colors flex items-center gap-1.5"
                title="Improve with AI"
              >
                <Wand2 className="w-3.5 h-3.5"/>
                <span className="text-[10px] font-bold">AI Improve</span>
              </button>
              <button
        ...
        <div className="relative">
          {isPreviewMode ? (
            <div
              className="prose prose-invert prose-sm max-w-none bg-slate-900/30 p-4 rounded-xl border border-slate-800/50 min-h-[120px]">
              {editedDescription ? (
            ...
            <textarea
              ...
              className="w-full min-h-[120px] bg-[#161B26] border border-slate-800 rounded-xl p-4 text-sm text-slate-300 placeholder:text-slate-600 focus:outline-none focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/20 transition-all resize-y"
            />
          )}
        </div>
      </div>
      ...
      <div className="space-y-4 pt-4 border-t border-slate-800/50">
        ...
        {isAddingDependency && (
          <div
            className="bg-slate-900/50 p-3 rounded-lg border border-slate-800 space-y-3 animate-in fade-in slide-in-from-top-2">
            <div className="flex gap-2">
              ...
              <div className="flex-1 relative">
                ...
                {dependencyQuery && (
                  <div
                    className="absolute top-full left-0 right-0 mt-1 max-h-40 overflow-y-auto bg-[#161B26] border border-slate-700 rounded shadow-xl z-20">
          ...
          {(["blocks", "blocked_by", "relates_to"] as const).map((type) => {
            ...
            return (
              <div key={type} className="space-y-1">
                <span
                  className={cn("text-[10px] font-bold uppercase tracking-wider px-2", color)}
                >
                  {label}
                </span>
