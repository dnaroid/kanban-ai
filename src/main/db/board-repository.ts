...
export class BoardRepository {
  getDefault(projectId: string): Board {
    ...
    const existingBoard = db.prepare('SELECT id, name FROM boards WHERE project_id = ? LIMIT 1').get(projectId) as { id: string, name: string } | undefined
  ...
  updateColumns(boardId: string, columns: { id?: string, name: string, orderIndex: number }[]): void {
    const db = dbManager.connect()
    const now = new Date().toISOString()
