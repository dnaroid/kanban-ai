...
export type RpcResponse = { ok: true; result: unknown } | { ok: false; error: { message: string; code?: string; details?: unknown } };
...
app.post("/rpc", async (req, res) => {
  ...
  if (!handler) return res.send({ ok:false, error:{ code:"NOT_FOUND", message:`Unknown method ${method}` } });
