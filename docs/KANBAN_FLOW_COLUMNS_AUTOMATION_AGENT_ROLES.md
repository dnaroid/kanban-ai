...
> Дата: 2026-01-30  
> Цель: предложить “типовой” набор колонок + правила автоматического перемещения задач + минимальный/оптимальный набор
> ролей агентов + полностью автоматизируемый end‑to‑end флоу: **требование → user story → ветка → реализация → PR/CI →
merge → обновление базы знаний**.
