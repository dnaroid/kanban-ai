...
import { ProjectsScreen } from "@web/screens/ProjectsScreen";
import { DiagnosticsScreen } from "@web/screens/DiagnosticsScreen";
import { BoardScreen } from "@web/screens/BoardScreen";
import { TimelineScreen } from "@web/screens/TimelineScreen";
import { SettingsScreen } from "@web/screens/SettingsScreen";
import { cn } from "@web/lib/utils";
import { Sidebar } from "@web/components/Sidebar";
import { SearchModal } from "@web/components/SearchModal";
import type { Screen } from "@web/types/screen";
import { uiStore } from "@web/stores/ui-store";

export default function App() {
	const [screen, setScreen] = useState<Screen>(
		uiStore.getSettings().currentScreen,
	);
	const [activeProject, setActiveProject] = useState<{
		id: string;
		name: string;
	} | null>(() => {
		const { lastProjectId, lastProjectName } = uiStore.getSettings();
		return lastProjectId && lastProjectName
			? { id: lastProjectId, name: lastProjectName }
			: null;
	});
	const [isSidebarCollapsed, setIsSidebarCollapsed] = useState(
		uiStore.getSettings().sidebarCollapsed,
	);
	const [isSearchOpen, setIsSearchOpen] = useState(false);

	useEffect(() => {
		uiStore.setSidebarCollapsed(isSidebarCollapsed);
	}, [isSidebarCollapsed]);

	useEffect(() => {
		uiStore.setCurrentScreen(screen);
		if (screen.id === "board" || screen.id === "timeline") {
			uiStore.setLastProject(screen.projectId, screen.projectName);
		}
	}, [screen]);

	useEffect(() => {
		if (activeProject) {
			uiStore.setLastProject(activeProject.id, activeProject.name);
		}
	}, [activeProject]);

	const openProjectBoard = (projectId: string) => {
		const name =
			activeProject?.id === projectId ? activeProject.name : projectId;
	...
	return (
		<div
			...
			style={
				{
					"--sidebar-width": isSidebarCollapsed ? "64px" : "256px",
			...
			>
				...
				<div
					className={cn(
						...
						screen.id !== "board" &&
							(screen.id === "settings" ? "px-8 pb-8 pt-2" : "p-8"),
					)}
				>
					{screen.id === "projects" && (
						<ProjectsScreen
