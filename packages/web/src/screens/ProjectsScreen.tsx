...
function ProjectCard({
  ...
  const handleRename = async () => {
    if (newName.trim() && newName !== project.name) {
      await window.api.project.update({ id: project.id, name: newName.trim() })
      onUpdate()
    }
    setIsEditing(false)
    setShowMenu(false)
  ...
  const handleColorChange = async (color: string) => {
    await window.api.project.update({ id: project.id, color })
    onUpdate()
    setShowColorPicker(false)
    setShowMenu(false)
  }
  ...
  return (
    <div
      onClick={() => !showMenu && !isEditing && onSelect(project.id, project.name)}
...
export function ProjectsScreen({ onProjectSelect }: ProjectsScreenProps) {
  ...
  const handleCreateProject = async (name: string, path: string, color: string) => {
    try {
      await window.api.project.create({ name, path, color })
          ...
          {projects.map((project) => (
            <ProjectCard
              ...
              onSelect={async (id, name) => {
                await window.api.appSetting.setLastProjectId({ projectId: id })
