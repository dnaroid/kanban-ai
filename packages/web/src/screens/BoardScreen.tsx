...
export function BoardScreen({ projectId }: BoardScreenProps) {
	const [viewMode, setViewMode] = useState<"board" | "list">(() => {
		return (uiStore.getSettings().boardViewMode as "board" | "list") || "board";
	});
	const {
		board,
		tasks,
	...
	}, [columns.length, tasks.length === 0]);

	useEffect(() => {
		uiStore.updateSettings({ boardViewMode: viewMode });
	}, [viewMode]);
	...
	return (
		<div className="flex flex-col h-full overflow-hidden">
									...
									{columns.map((column) => (
										<SortableColumn
											...
											tasks={tasks
												...
												.sort(
													(a, b) =>
														(a.orderInColumn || 0) - (b.orderInColumn || 0),
												)}
			...
			<TaskDrawer
				...
				columnName={
					board?.columns?.find((c) => c.id === selectedTask?.columnId)?.name
				}
