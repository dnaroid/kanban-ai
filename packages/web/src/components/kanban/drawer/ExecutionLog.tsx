...
import {
	...
	TextPart,
	ToolPart,
} from "@web/components/chat/MessageParts";
...
export function ExecutionLog({
	...
	const handleSendMessage = async () => {
		...
		try {
			await window.api.opencode.sendMessage({
	...
	useEffect(() => {
		...
		const pollSessionId = async () => {
			try {
				...
				// Run ещё не сохранён в БД - retry через 1 сек
			}
			if (isActive) {
		...
		const updateMessagePart = (messageId: string, part: Part) => {
			...
			setEvents((prev) => {
				...
				if (existingIndex === -1) {
					...
					console.log(
						"[ExecutionLog] Creating new message with part:",
						messageId,
					);
					seenMessageIdsRef.current.add(id);
				...
				updated[existingIndex] = updatedEvent;
				console.log(
					"[ExecutionLog] Message updated, total parts:",
					newParts.length,
				);
				return updated;
			});
