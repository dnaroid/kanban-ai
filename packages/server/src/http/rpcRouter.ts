...
async function resolveBrowsePath(
	...
	if (
		typeof lastSelectedPath === "string" &&
		lastSelectedPath.trim().length > 0
	) {
	...
	if (lastProjectId) {
		const projectResult =
			await container.getProjectByIdUseCase.execute(lastProjectId);
...
export function createRpcRouter(
	...
	router.set(
		"project:browseDirectory",
		async (params: BrowseDirectoryParams = {}) => {
			const currentPath = await resolveBrowsePath(
				container,
				params.path,
				params.lastSelectedPath,
			);
	...
	router.set("opencode:refreshModels", async () => {
		...
		const models = Array.from(variantsByModel.entries()).map(
			([name, variants]) => ({
				name,
	...
	router.set("opencode:toggleModel", async (params) => {
		const updatedModel = container.updateModelEnabled(
			params.name,
			params.enabled,
		);
		if (!updatedModel) {
	...
	router.set("opencode:updateModelDifficulty", async (params) => {
		const updatedModel = container.updateModelDifficulty(
			params.name,
			params.difficulty,
		);
		if (!updatedModel) {
	...
	router.set("ohMyOpencode:readConfig", async (params) => {
		...
		const { buildOhMyOpencodeModelFields } = await import(
			"../oh-my-opencode/config-utils"
		);
	...
	router.set("ohMyOpencode:saveConfig", async (params) => {
		const { parse } = await import("jsonc-parser");
		const { ORIGINAL_PRESET_NAME, isPlainObject, mergeInPlace } = await import(
			"../oh-my-opencode/config-utils"
		);
		const fileContent = await fs.readFile(params.path, "utf-8");
	...
	router.set("ohMyOpencode:listPresets", async (params) => {
		const { PRESET_SUFFIX, ORIGINAL_PRESET_NAME } = await import(
			"../oh-my-opencode/config-utils"
		);
	...
	router.set("ohMyOpencode:loadPreset", async (params) => {
		const { parse } = await import("jsonc-parser");
		const { PRESET_SUFFIX, buildOhMyOpencodeModelFields } = await import(
			"../oh-my-opencode/config-utils"
		);
		const presetPath = path.join(
			path.dirname(params.path),
			`${params.presetName}${PRESET_SUFFIX}`,
		);
