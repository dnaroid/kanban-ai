...
export class OpenCodeSessionWorker {
	...
	private emitUpdate(update: SessionUpdate): void {
		publishEvent("run:onEvent", update);
	...
	private async trackSession(
		...
		const timeoutMs =
			input.kind === "task-description-improve" ? 120000 : 3600000;
		const initialDelayMs =
			input.kind === "task-description-improve" ? 6000 : 3000;
			...
			while (Date.now() < deadline) {
				...
				if (input.kind === "task-description-improve") {
					...
					if (parsed.difficulty) {
						...
						const model = opencodeModelRepo.getModelForDifficulty(
							parsed.difficulty,
						);
						if (model) {
	...
	private async finishSession(
		...
		if (deleteArtifacts) {
			try {
				...
				console.warn(
					"[OpenCodeSessionWorker] finishSession:deleteSession:failed",
					{
						sessionId,
						error: error instanceof Error ? error.message : String(error),
					},
	...
	private findLatestStatus(
		...
		for (let index = messages.length - 1; index >= 0; index -= 1) {
			...
			for (const part of parts as any[]) {
				...
				const content =
					this.extractAssistantText(message) || part.text.trim() || "";
	...
	private parseUserStoryResponse(content: string): {
		...
		const rawStory = storyMatch
			? storyMatch[1]
			: content.replace(/<META>[\s\S]*?<\/META>/i, "");

		const description = rawStory.trim() || content.trim();
		const titleMatch = rawStory.match(
			/^[\s>*_-]*\*{0,2}Название\*{0,2}:\s*(.+)$/im,
		);
	...
	private resolveDoneColumnId(taskId: string): string | null {
		...
		const lastByOrder = columns.reduce((acc, col) =>
			col.orderIndex > acc.orderIndex ? col : acc,
		);
