...
const resolveOpencodeEventList = (
	...
	console.log(
		"[OpenCodeSessionManager] resolveOpencodeEventList: client keys:",
		Object.keys(root),
	);
	...
	if (event && typeof event["subscribe"] === "function") {
		console.log(
			"[OpenCodeSessionManager] resolveOpencodeEventList: found client.event.subscribe",
		);
	...
	console.log(
		"[OpenCodeSessionManager] resolveOpencodeEventList: subscribe method not found",
	);
	return null;
};
...
export class OpenCodeSessionManager {
	...
	>();
	private directoryClients = new Map<
		string,
		ReturnType<typeof createOpencodeClient>
	>();
	...
	private dispatchSessionEvent(sessionID: string, event: SessionEvent): void {
		try {
			publishEvent("opencode:onEvent", { sessionId: sessionID, event });
		} catch (error) {
			console.error(
				"[OpenCodeSessionManager] Failed to publish event to EventBus",
				{
					sessionID,
		...
		for (const subscriber of subscribers.values()) {
			try {
				...
				console.warn(
					"[OpenCodeSessionManager] Session event subscriber failed",
					{
						sessionID,
						eventType: event.type,
						error: error instanceof Error ? error.message : String(error),
	...
	async sendPrompt(
		...
		const directory =
			sessionInfo?.directory || (await this.resolveSessionDirectory(sessionID));
		if (!directory) {
		...
		try {
			...
			if (response.error) {
				console.error(
					"[OpenCodeSessionManager] sendPrompt ERROR:",
					response.error,
				);
	...
	async sendPromptAsync(
		...
		if (response.error) {
			console.error(
				"[OpenCodeSessionManager] sendPromptAsync ERROR:",
				response.error,
			);
	...
	async getMessages(
		...
		const sessionInfo = this.activeSessions.get(sessionID);
		if (!sessionInfo) {
			const messages = await this.storageReader.getMessagesFromFilesystem(
				sessionID,
				limit,
			);
	...
	async getTodos(sessionID: string): Promise<SessionTodo[]> {
		...
		if (!directory) {
			console.warn(
				`[OpenCodeSessionManager] getTodos: session not found ${sessionID}`,
			);
			return [];
		}
		...
		try {
			...
			console.error(
				`[OpenCodeSessionManager] Failed to fetch todos for ${sessionID}:`,
				error,
			);
			return [];
		}
	...
	> {
		...
		if (response.error) {
			...
			if (message.includes("NotFoundError") || message.includes("ENOENT")) {
				console.log(
					"[OpenCodeSessionManager] getMessagesRaw: SDK error, loading from filesystem",
					{
						sessionID,
					},
				);
	...
	async subscribeToSessionEvents(
		...
		if (this.eventAbortControllers.has(sessionID)) {
			console.log(
				`[OpenCodeSessionManager] Already subscribed to session ${sessionID}`,
			);
			return;
		}
		...
		const processEvents = async () => {
			try {
				...
				console.log(
					`[OpenCodeSessionManager] Calling listEvents for session ${sessionID}`,
				);
				const streamPromise = listEvents({}, { signal });
				console.log(
					`[OpenCodeSessionManager] Stream promise created, awaiting...`,
				);

				const streamResult = await Promise.resolve(streamPromise).catch(
					(error) => {
						console.error(
				...
				const stream =
					(streamResult as any).stream ??
					(streamResult as any).data ??
					streamResult;
				console.log(
				...
				this.eventSubscribers.delete(sessionID);
				console.log(
					`[OpenCodeSessionManager] Event stream ended for session ${sessionID}`,
				);
			}
		};
		...
		console.log(
			`[OpenCodeSessionManager] Subscribed to events for session ${sessionID}`,
		);
	}
	...
	async unsubscribeFromSessionEvents(
		...
		const hasSubscribers =
			(this.eventSubscribers.get(sessionID)?.size ?? 0) > 0;
		if (hasSubscribers) {
			return;
		}
		...
		console.log(
			`[OpenCodeSessionManager] Unsubscribed from events for session ${sessionID}`,
		);
		this.eventAbortControllers.delete(sessionID);
	...
	private async logProvidersInternal(
		...
		try {
			...
			if (providers.data) {
				...
				for (const provider of allProviders) {
					...
					for (const model of models) {
						...
						console.log(
							`    - ${modelInfo.name || "unknown"}${reasoning}${status}`,
						);
			...
			console.warn(
				"[OpenCodeSessionManager] Failed to fetch providers:",
				error,
			);
		}
	}
	...
	private async getSessionDirectoryFromStorage(
		sessionID: string,
	): Promise<string | null> {
	...
	private shouldHandleSessionEvent(sessionID: string, event: Event): boolean {
		switch (event.type) {
			...
			case "message.removed":
				return true;
			case "message.part.updated": {
				const partSessionId = (event.properties.part as { sessionID?: string })
					.sessionID;
