...
import { createAppContext } from "../../ipc/composition/create-app-context";
...
describe("resolveInProgressColumnId", () => {
	...
	it("prefers system_key=in_progress over orderIndex fallback", () => {
		...
		db.prepare(
			"UPDATE board_columns SET system_key = ? WHERE board_id = ?",
		).run("", board.id);
		db.prepare("UPDATE board_columns SET system_key = ? WHERE id = ?").run(
			"in_progress",
			done.id,
		);
