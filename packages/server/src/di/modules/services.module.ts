...
import type { AgentRoleRepository } from '@server/db/agent-role-repository'
import type { AppSettingsRepository } from '@server/db/app-settings-repository'
import type { ArtifactRepository } from '@server/db/artifact-repository'
import type { BoardRepository } from '@server/db/board-repository'
import type { OpencodeModelRepository } from '@server/db/opencode-model-repository'
import type { RunEventRepository } from '@server/db/run-event-repository'
import type { TagRepository } from '@server/db/tag-repository'
import type { TaskRepository } from '@server/db/task-repository'
import type { TaskScheduleRepository } from '@server/db/task-schedule-repository'
import type { PathsService } from '@server/paths'
import type { DialogService } from '@server/services/DialogService'
  ...
  return {
    ...
    createOpencodeClientInstance: (projectPath?: string) => {
      ...
      return createOpencodeClient({
        baseUrl,
        throwOnError: true,
        directory: projectPath ?? paths.getDataDir(),
      })
    },
    queryTasks: (query: string, filters?: any, limit = 50, offset = 0) =>
      searchService.queryTasks(query, filters, limit, offset),
    queryRuns: (query: string, filters?: any, limit = 50, offset = 0) =>
      searchService.queryRuns(query, filters, limit, offset),
    queryArtifacts: (query: string, filters?: any, limit = 50, offset = 0) =>
      searchService.queryArtifacts(query, filters, limit, offset),
    selectFolder: () => dialogService.selectFolder(),
  }
}
