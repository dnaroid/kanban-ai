...
5) В поле agentRoleId укажи один id из списка доступных ролей. Выбирай роль по смыслу задачи (кто должен исполнять реализацию).
6) Никогда не выбирай agentRoleId только потому, что генератор запущен в этой роли.
7) Роль ba выбирай только для задач анализа/требований/документации без изменений кода. Если в задаче есть реализация (UI/API/бэкенд/тесты/инфра/безопасность/данные), выбирай профильного исполнителя (fe/be/qa/sre/sec/data/tl).
8) Последняя строка ответа должна быть marker-статусом:
