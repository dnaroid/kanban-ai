...
8) Роль fe (фронтенд) выбирай ТОЛЬКО если задача сфокусирована на UI/UX: компоненты, стили, анимации, верстка, дизайн-система, адаптивность, интерактивность интерфейса. Задачи, связанные с логикой, API, архитектурой, БД, конфигурацией, инфраструктурой, тестами — НЕ являются фронтенд-задачами, даже если они косвенно влияют на UI. Для таких задач выбирай be, tl или другого профильного исполнителя.
9) Агент-исполнитель может сам делегировать часть работы другому агенту (в т.ч. fe) в процессе выполнения. Не назначай fe «на всякий случай» — если задача не сфокусирована на UI, назначь tl или be.
10) Для задач без чёткой специализации (архитектура, рефакторинг, смешанная логика) выбирай tl как универсального исполнителя.
11) Последняя строка ответа должна быть marker-статусом:
