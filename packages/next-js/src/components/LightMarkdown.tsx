			...
			case "link": {
				...
				const handleClick = (e: React.MouseEvent) => {
					if (isFile) {
						...
						try {
							const decodedPath = decodeURI(filePath).replace(
								/^\/([A-Za-z]:)/,
								"$1",
							);
							api.app.openPath(decodedPath);
						} catch (err) {
				...
				return (
					<span
						key={i}
						className="inline-flex items-center mx-0.5 my-0.5 group/pill"
					>
						<a
							href={n.url}
							target="_blank"
...
function renderListItem(
	...
	return (
		<div className="flex gap-2 items-start">
			...
			<div
				className={`flex-1 ${it.checked ? "line-through text-slate-500" : ""}`}
			>
				{content}
			</div>
		</div>
