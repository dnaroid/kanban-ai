...
import type { KanbanTask, TaskLink, TaskLinkType } from "@/types/kanban";
...
type DependencyRelationship = keyof typeof dependencyRelationshipConfig;

function isDependencyRelationship(
	value: string,
): value is DependencyRelationship {
	return value in dependencyRelationshipConfig;
}
...
export function TaskDetailsDependencies({
	...
	const [dependencyRelationship, setDependencyRelationship] =
		useState<DependencyRelationship>("blocks");

	const fetchDependencyData = useCallback(async () => {
		if (!task.boardId) return; // Cannot list by board if boardId missing
		setIsLoadingDependencies(true);
		setDependencyError(null);
		try {
			const [tasksResult, depsRes] = await Promise.all([
				api.task.listByBoard(task.boardId),
				api.deps.list({ taskId: task.id }),
			]);
			setBoardTasks(tasksResult.tasks);
	...
	const handleAddDependency = async () => {
		...
		let linkType: TaskLinkType =
			dependencyRelationship === "relates" ? "relates" : "blocks";
		...
		try {
			await api.deps.add({ fromTaskId, toTaskId, type: linkType });
	...
	const handleDependencyRelationshipChange = (value: string) => {
		if (!isDependencyRelationship(value)) return;
		setDependencyRelationship(value);
	};
	...
	return (
		<div className="space-y-4 pt-4 px-6 pb-6">
				...
				<div className="bg-slate-900/50 p-3 rounded-lg border border-slate-800 space-y-3 animate-in fade-in slide-in-from-top-2">
					<div className="flex gap-4 items-end">
						<PillSelect
							...
							options={dependencyRelationshipConfig}
							onChange={handleDependencyRelationshipChange}
						/>
								...
								<div className="absolute top-full left-0 right-0 mt-1 max-h-40 overflow-y-auto bg-[#161B26] border border-slate-700 rounded shadow-xl z-20">
									{dependencyResults.map((t) => (
										<button
											key={t.id}
											type="button"
											className="w-full px-2 py-1.5 text-xs text-slate-300 hover:bg-slate-800 cursor-pointer flex justify-between text-left"
											onClick={() => {
			...
			<div className="space-y-2">
				{(["blocks", "blocked_by", "relates"] as const).map((type) => {
					const links =
						type === "blocks"
							? blocksLinks
							: type === "blocked_by"
